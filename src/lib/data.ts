...
export const barangayZipCodes: Record<string, Record<string, string>> = {
  "QUEZON CITY": {
    "COMMONWEALTH": "1121",
    "BATASAN HILLS": "1126",
    "HOLY SPIRIT": "1127",
...
export const getZipCode = (city: string, barangay?: string): string => {
  // STRICT: No barangay selection means no zip code default
  if (!barangay || barangay.trim() === "") return "";
  ...
  if (barangayZipCodes[lookupCity] && barangayZipCodes[lookupCity][lookupBrgy]) {
    return barangayZipCodes[lookupCity][lookupBrgy];
  }
  
  // Fallback to the general city code ONLY if a barangay was provided but not found in specific mapping
